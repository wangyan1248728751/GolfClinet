...
namespace Golf
{
    public class SettingForm : UGuiForm
    {
		...
		protected internal override void OnOpen(object userData)
		{
			...
			saveBtn.AddClick(Save);
			cancelBtn.AddClick(Cancel);

			string boxNameStr = GameEntry.GameData.boxName;

			if (boxNameStr.Contains("SKYTRAK_"))
				boxNameStr = boxNameStr.Replace("SKYTRAK_", "");

			boxName.text = boxNameStr;
			shopId.text = GameEntry.GameData.ShopName.ToString();
		}

		void Save()
		{
			GameEntry.GameData.boxName = "SKYTRAK_" + boxName.text.ToUpper();
			if (!string.IsNullOrEmpty(shopId.text))
				GameEntry.GameData.ShopName = int.Parse(shopId.text);
			Close();
		}
